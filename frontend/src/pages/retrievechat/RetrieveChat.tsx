...
import styles from "./RetrieveChat.module.css";
...
export function Component(): JSX.Element {
    const chat_type = CHAT_TYPE.RETRIEVE;
    ...
    const makeApiRequest = async (question: string, files: UploadFile[]) => {
        ...
        try {
            ...
            formData.append("history", JSON.stringify([...history, { user: question, bot: undefined }]));
            if (files) {
                files.map((file, index) => {
                    formData.append(`file${index}`, file as RcFile);
                });
            }
            const result = await API.retrieveChatApi(formData);
...
Component.displayName = "RetrieveChat";
